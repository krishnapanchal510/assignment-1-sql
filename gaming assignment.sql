...
0    result VARCHAR(10) CHECK (result IN ('Win', 'Loss', 'Draw')),
...
-- Q11. Insert 3 players into the players table
INSERT INTO players (username, email, full_name, country, registration_date, level, coins, status) VALUES
('SuperGamer', 'supergamer@gmail.com', 'Dev Patel', 'India', '2024-04-10', 10, 1200, 'Active'),
('SpeedyFox',  'speedyfox@yahoo.com',  'Alex Mercer', 'USA', '2024-04-12', 15, 2300, 'Active'),
('DarkKnight', 'darkknight@gmail.com', 'Sanjay Dutt', 'India', '2024-04-15', 20, 3100, 'Active');

-- Q12. Insert 2 games into the games table
INSERT INTO games (game_name, genre, release_date, developer, rating, price, downloads) VALUES
('Shadow Strike',  'Action',   '2024-01-15', 'Tencent Games', 4.6, 499.00, 1000000),
('Empire Builder', 'Strategy', '2023-11-20', 'Firaxis',       4.4, 899.00, 850000);

-- Q13. Insert 5 match records into the matches table
INSERT INTO matches (player_id, game_id, match_date, score, duration_min, result, mode) VALUES
(1, 1, '2024-04-10 14:00:00', 2500, 30, 'Win',  'Solo'),
(2, 2, '2024-04-11 16:30:00', 1800, 45, 'Loss', 'Team'),
(3, 4, '2024-04-12 18:00:00', 3200, 90, 'Win',  'Solo'),
(4, 3, '2024-04-13 12:00:00', 1400, 60, 'Draw', 'Solo'),
(5, 6, '2024-04-14 20:00:00', 1100, 50, 'Loss', 'Duo');

-- Q14. Insert a tournament record
INSERT INTO tournaments (tournament_name, game_id, start_date, end_date, prize_pool, winner_id, status) VALUES
('Battleground Invitational', 1, '2024-05-10', '2024-05-25', 100000.00, NULL, 'Ongoing');

-- Q15. Update the level of player_id = 5 to 25 and increase coins by 500
UPDATE players 
SET level = 25, coins = coins + 500 
WHERE player_id = 5;

-- Q16. Update the status of players registered before '2023-01-01' with level < 5 to 'Inactive'
UPDATE players 
SET status = 'Inactive' 
...
-- Q18. Update tournament_id = 3 winner_id to 10 and status to 'Completed'
UPDATE tournaments 
SET winner_id = 10, status = 'Completed' 
WHERE tournament_id = 3;

-- Q19. Delete all matches where result = 'Loss' and score < 100
DELETE FROM matches 
WHERE result = 'Loss' AND score < 100;

-- Q20. Delete all achievements earned more than 2 years ago (T-SQL DATEADD)
DELETE FROM achievements 
WHERE earned_date < DATEADD(year, -2, CAST(GETDATE() AS DATE));




-- Q21. Select all columns from players
SELECT * FROM players;

-- Q22. Retrieve username, email, and country of all players
...
-- Q24. List all games with a rating >= 4.0
SELECT * FROM games WHERE rating >= 4.0;

-- Q25. Display all matches where result was 'Win'
SELECT * FROM matches WHERE result = 'Win';

-- Q26. Find players whose level is between 10 and 50 inclusive
SELECT * FROM players WHERE level BETWEEN 10 AND 50;

-- Q27. Retrieve all games where price > 500
...
-- Q29. Find players whose username starts with 'Pro'
SELECT * FROM players WHERE username LIKE 'Pro%';

-- Q30. Display games where genre is either 'Action' or 'Strategy'
SELECT * FROM games WHERE genre IN ('Action', 'Strategy');

-- Q31. Retrieve purchases made using 'UPI' or 'Card'
SELECT * FROM purchases WHERE payment_method IN ('UPI', 'Card');

-- Q32. Find achievements with rarity 'Legendary'
SELECT * FROM achievements WHERE rarity = 'Legendary';

-- Q33. List players whose email ends with '@gmail.com'
SELECT * FROM players WHERE email LIKE '%@gmail.com';

-- Q34. Display DISTINCT countries from players
SELECT DISTINCT country FROM players;

-- Q35. Find matches where mode is NOT 'Solo'
SELECT * FROM matches WHERE NOT mode = 'Solo';




-- Q36. List players ordered by level descending
SELECT * FROM players ORDER BY level DESC;

-- Q37. Display games ordered by rating DESC, then price ASC
SELECT * FROM games ORDER BY rating DESC, price ASC;

-- Q38. Retrieve top 10 matches ordered by score descending (T-SQL TOP)
SELECT TOP 10 * FROM matches ORDER BY score DESC;

-- Q39. List tournaments ordered by prize_pool descending
SELECT * FROM tournaments ORDER BY prize_pool DESC;

-- Q40. Display purchases ordered by purchase_date descending
SELECT * FROM purchases ORDER BY purchase_date DESC;



-- Section E-1: INNER JOIN
-- Q41. List all matches with player username and game name
-- INNER JOIN
...
-- Q42. Display achievements with player username and game name
-- INNER JOIN
...
-- Q43. List purchases with player username, game name, item name, amount
-- INNER JOIN
SELECT p.username, g.game_name, pur.item_name, pur.amount, pur.purchase_date 
FROM purchases pur 
INNER JOIN players p ON pur.player_id = p.player_id 
INNER JOIN games g ON pur.game_id = g.game_id;

-- Q44. Show tournaments with game name and winner username
-- INNER JOIN
SELECT t.tournament_name, g.game_name, p.username AS winner_username, t.prize_pool, t.status 
FROM tournaments t 
INNER JOIN games g ON t.game_id = g.game_id 
INNER JOIN players p ON t.winner_id = p.player_id;

-- Q45. Find matches where score > 1000 with player and game details
-- INNER JOIN
...
-- Q46. Find players who have NEVER played any match
-- LEFT JOIN
...
-- Q47. List games along with match counts (including zero)
-- LEFT JOIN
...
-- Q48. Find players who have NOT earned any achievement
-- LEFT JOIN
...
-- Q49. Display games and total purchase revenue (include 0 for no purchases)
-- LEFT JOIN
SELECT g.game_name, COALESCE(SUM(p.amount), 0) AS total_revenue 
FROM games g 
LEFT JOIN purchases p ON g.game_id = p.game_id 
GROUP BY g.game_id, g.game_name;

-- Q50. List tournaments with winner username (include NULL for ongoing/upcoming)
-- LEFT JOIN
...
-- Q51. Comprehensive match report joining players, matches, and games
-- INNER JOIN
...
-- Q52. Complete tournament report
...
-- Q53. Comprehensive purchase report joining all 3 tables
-- INNER JOIN
SELECT p.username, p.country, g.game_name, pur.item_name, pur.amount, pur.payment_method, pur.purchase_date 
FROM purchases pur 
INNER JOIN players p ON pur.player_id = p.player_id 
INNER JOIN games g ON pur.game_id = g.game_id;

-- Q54. Find players from 'India' who won matches in 'Action' games
-- INNER JOIN
SELECT p.username, g.game_name, m.score, m.match_date 
FROM matches m 
INNER JOIN players p ON m.player_id = p.player_id 
...
-- Q55. Achievements earned in games with rating > 4.0
-- INNER JOIN
...
-- Q56. Find pairs of players from the same country
-- SELF JOIN
SELECT p1.username AS player1_username, p2.username AS player2_username, p1.country 
FROM players p1 
INNER JOIN players p2 ON p1.country = p2.country AND p1.player_id < p2.player_id;

-- Q57. Find pairs of games released in the same year
-- SELF JOIN
SELECT g1.game_name AS game1_name, g2.game_name AS game2_name, YEAR(g1.release_date) AS release_year 
FROM games g1 
INNER JOIN games g2 ON YEAR(g1.release_date) = YEAR(g2.release_date) AND g1.game_id < g2.game_id;

-- Section E-5: FULL OUTER JOIN & CROSS JOIN
-- Q58. FULL OUTER JOIN between players and matches
-- FULL OUTER JOIN
SELECT p.player_id, p.username, m.match_id, m.score, m.result 
FROM players p 
FULL OUTER JOIN matches m ON p.player_id = m.player_id;

-- Q59. CROSS JOIN between games and derived match modes
-- CROSS JOIN
SELECT g.game_name, m.mode 
FROM games g 
CROSS JOIN (SELECT 'Solo' AS mode UNION SELECT 'Duo' UNION SELECT 'Squad' UNION SELECT 'Team') m;

-- Q60. CROSS JOIN between players and games limited to 20 rows (T-SQL TOP)
-- CROSS JOIN
...
-- Q61. Total number of registered players
SELECT COUNT(*) AS total_players FROM players;

-- Q62. Highest score, lowest score, and average score across all matches
SELECT MAX(score) AS highest_score, MIN(score) AS lowest_score, AVG(CAST(score AS FLOAT)) AS average_score FROM matches;

-- Q63. Total revenue generated from all purchases
SELECT SUM(amount) AS total_revenue FROM purchases;

-- Q64. Average prize pool across completed tournaments
SELECT AVG(prize_pool) AS avg_completed_prize_pool FROM tournaments WHERE status = 'Completed';

-- Q65. Count total number of achievements earned per rarity level
SELECT rarity, COUNT(*) AS achievement_count FROM achievements GROUP BY rarity;




-- Q66. Total matches played by each player ordered by match_count descending
...
-- Q67. Total revenue generated from purchases for each game ordered descending
SELECT g.game_name, COALESCE(SUM(p.amount), 0) AS total_revenue 
FROM games g 
LEFT JOIN purchases p ON g.game_id = p.game_id 
GROUP BY g.game_id, g.game_name 
...
-- Q68. Group matches by result and count matches per outcome
...
-- Q70. List games played in more than 10 matches ordered by total_matches descending
SELECT g.game_name, COUNT(m.match_id) AS total_matches 
FROM games g 
INNER JOIN matches m ON g.game_id = m.game_id 
GROUP BY g.game_id, g.game_name 
